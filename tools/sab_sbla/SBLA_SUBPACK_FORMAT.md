...
DIRECTORY  (object: @0x20; streamblock: @0x44)  — N × 0x18 (24-byte) descriptors:
  ...
  +0x04  u32   offset      blob offset (object: absolute file offset, base = leadCompSize region;
                           streamblock: relative, base 0). offset[0]=leadCompSize (obj) / 0 (sb);
...
BODY     compressed blocks back-to-back, one per real record; total = Σ compSize
         object packs: at the absolute base; streamblock packs: at EOF − Σ compSize
TRAILING footer/padding (preserved verbatim)
```
