...
- ⚠️ **REFUTED 2026-07-24.** ~~**FUN_00422e90** — `WSVehicleSkidManager` … Manages skid-mark decals/particles per wheel.~~ `FUN_00422e90` (size 5730, sole caller `FUN_0042687b`) is the **renderer's engine-wide drawlist/bucket registrar**, not a vehicle class. Its body holds **124 distinct strings**, overwhelmingly render passes and buckets: `Begin Far Zpass`, `Begin Near Color Pass(%d)`, `Begin ShadowMap(%d)`, `Begin Terrain Screen Pass`, `AtmosphereBucket`, `AtmosphereDrawlist`, `ClamberingHUDBucket`, `Decals Bucket`, `CloudsDrawlistHighWTF`, `WaterReflection*`, … `"VehicleSkid"` is **one bucket name among them** — the skid drawlist registration, not a `WSVehicleSkidManager` implementation. [`world-water.md`](world-water.md) independently names the same VA correctly as `World_BuildRenderPasses`; prefer that reading.
...
- ⚠️ **REFUTED 2026-07-24.** ~~**FUN_00762fa0** — `Vehicle.cpp` Lua interface registrar … Registers the race-callback table.~~ It is a **single binding implementation**: `data/lua_registration_map.tsv` gives `Vehicle.StartPlayback  impl_va=00762fa0`. (It does assert the `Script\Interface\Vehicle.cpp` path — every binding in that file does, which is what misled the original reading.)
- ⚠️ **REFUTED 2026-07-24.** ~~**FUN_007628e0** — Race-callback binding block … for the full race-event set.~~ Also a single binding: `Vehicle.SetRaceLoadedCallback  impl_va=007628e0`. Each race callback has its own implementation — `SetRaceStartCallback` `0x00762950`, `SetRaceFinishedCallback` `0x007629c0`, and so on through `0x00762c20`. There is no one "binding block".
