...
- **No RTTI vtable→VA map yet.** Element classes are pinned only where a `WSCinemaElement.cpp` assertion string names the method; the vtable slot each executor occupies is unverified.
...
- FUN_009ccf30 = WSMissionMessengerManager::AttemptDeliveryWithCurrentMessenger (assertion string at ~861237, size=6012, caller 0x0043909f) directly drives cinematics: calls FUN_0094f710(name,2,...) to PLAY and FUN_0094cc50() to start the cinematic camera. Mission/Messenger -> Cinematics seam the doc omits (doc only credits the Lua glue as callers of FUN_0094f710/FUN_0094cc50).
