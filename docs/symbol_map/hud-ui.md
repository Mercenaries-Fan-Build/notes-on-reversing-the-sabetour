...
- **`LocalizedString_Fetch`** (`FUN_00db7e10` → `FUN_00db7c10`): string-table lookup by id, called throughout the HUD with keys `HUD.Saving`, `HUD.Locked`, `HUD.AmmoFull`, `HUD.PickedUp`, `LanguageSelection`, `LegalInfo`. Feeds the string `SetVariable` calls.
- **Scaleform GFx library**: named symbol `GFxLoader::~GFxLoader @0x00b95400` survives; `FUN_00ba2a00` loads movies by filename (`IME.swf`, `gfxfontlib.swf`). `WSHUDFileOpener` / `WSHUDMemFile` implement the GFx `GFileOpener` interface; `WSHudFlashRenderer` / `WSHudFlashTexture` bridge GFx rendering to the engine's `WSGfxSubsystem`(`Job`).
...
## Gaps
See structured `gaps`: no RTTI vtable→VA map yet, so the 41 widget classes in the `+0x34` array are not individually resolved; most `HUD.*` binding backends and the un-named template cases (1,2,7,8,9,13,14,16-19) are unpinned; the flash-renderer / GfxSubsystem GPU path carries no name strings.
